...
// Sections shown on the "Trending" (All) tab.
// wc2026live channels are folded into the fifa2026 bucket so they appear together.
const TRENDING_SECTIONS = [
  { id: 'fifa2026',   label: 'FIFA & WC 2026', icon: '🏆' },
...
// ── Trending sections (shown on "All" tab only) ─────────────────────────────────

function TrendingSections({ channels, setActiveCategory }) {
  const [expanded, setExpanded] = useState(null)
  const [gridPage, setGridPage] = useState(1)

  // Group by category; fold wc2026live into fifa2026 so they appear in one row.
  const grouped = useMemo(() => {
    const map = {}
    for (const ch of channels) {
      const cat = ch.category === 'wc2026live' ? 'fifa2026' : (ch.category || 'multi')
      if (!map[cat]) map[cat] = []
      map[cat].push(ch)
    }
    if (map.fifa2026) {
...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    } else if (activeCategory === 'fifa2026') {
      // Combine FIFA + WC 2026 Live channels under the merged tab
      list = list.filter((c) => c.category === 'fifa2026' || c.category === 'wc2026live')
  ...
  // Only the "Trending" (All) tab gets the sectioned view; every other
  // category shows a flat paginated grid — simpler and faster.
  const showSections = activeCategory === 'all' && !searchQuery.trim() && !qualityFilter

  // Quality counts computed from the pre-quality-filtered list
  const qualityCounts = useMemo(() => {
    let base = channels
    if (activeCategory === 'fancode')       base = base.filter((c) => c.key?.startsWith('fc_'))
    else if (activeCategory === 'sonyliv')  base = base.filter((c) => c.key?.startsWith('sl_'))
    else if (activeCategory === 'tataplay') base = base.filter((c) => c.key?.startsWith('tp_'))
    else if (activeCategory === 'fifa2026') base = base.filter((c) => c.category === 'fifa2026' || c.category === 'wc2026live')
    else if (activeCategory !== 'all')      base = base.filter((c) => c.category === activeCategory)
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase()
      base = base.filter((c) => c.name?.toLowerCase().includes(q) || c.currentMatch?.toLowerCase().includes(q) || c.category?.toLowerCase().includes(q))
    }
    return {
      null: base.length,
      HD: base.filter((c) => { const b = c.badge?.toUpperCase(); return b === 'HD' || b === '1080P' || b === '720P' }).length,
      '4K': base.filter((c) => c.badge?.toUpperCase() === '4K').length,
      '2K': base.filter((c) => c.badge?.toUpperCase() === '2K').length,
    }
  ...
  return (
    <main ref={containerRef} className="flex-1 overflow-y-auto bg-black pb-safe no-scrollbar">
      ...
      <div className="flex items-center gap-2 px-4 md:px-6 pb-3">
        ...
        <div className="flex gap-1.5">
          {QUALITY_FILTERS.map(({ label, value }) => {
            const cnt = qualityCounts[value]
            return (
              <button
                key={label}
                onClick={() => setQualityFilter(value)}
                disabled={value !== null && cnt === 0}
      ...
      {/* Favourites row — only on the All tab with no active filters */}
